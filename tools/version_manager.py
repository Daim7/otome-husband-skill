...
负责老公 Skill 文件的版本存档和回滚。

用法：
  python version_manager.py --action list --slug ling-xiao --base-dir ./husbands
  python version_manager.py --action rollback --slug ling-xiao --version v2 --base-dir ./husbands
  python version_manager.py --action cleanup --slug ling-xiao --base-dir ./husbands
"""
...
def list_versions(skill_dir: Path) -> list:
    ...
    for v_dir in sorted(versions_dir.iterdir()):
        ...
        archived_at = datetime.fromtimestamp(mtime, tz=timezone.utc).strftime(
            "%Y-%m-%d %H:%M"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="老公 Skill 版本管理器")
    parser.add_argument("--action", required=True, choices=["list", "rollback", "cleanup"])
    parser.add_argument("--slug", required=True, help="老公 slug")
    ...
    if args.action == "list":
        ...
        if not versions:
            print(f"老公 [{args.slug}] 暂无历史版本存档")
        else:
            print(f"老公 [{args.slug}] 的版本历史：\n")
            for v in versions:
                print(
                    f"  {v['version']} 存档时间: {v['archived_at']} "
                    f"文件: {', '.join(v['files'])}"
                )
