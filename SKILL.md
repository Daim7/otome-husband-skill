...
### Step 2：原材料导入（可选，但强烈推荐！）

用「乙女抽卡池」语气询问，选项清晰：

```
想让老公更像你心目中的「他」吗？导入素材可以让 AI 学会他的说话方式！

  [A] 💬 导入聊天记录（最佳！）
      导入你和某个人的聊天记录，AI 会提取他的说话方式
      让生成的老公说话完全还原他的语感
      支持：微信导出 / QQ / 截图 / 直接粘贴
...
💡 Tip：导入聊天记录/台词可以让老公说话完全还原「他」的语感——
   口头禅、用词习惯、句式特征、emoji 风格全部提取！
```

- **A（聊天记录导入）**：
  - `Read` 聊天文件 → 参考 `${CLAUDE_SKILL_DIR}/prompts/chat_style_analyzer.md` 分析
  - 提取：用词指纹、句式指纹、情感表达方式、独特表达
  - 生成 **聊天风格档案**，注入到 dialogue.md（话术） + persona.md Layer 4（语言指纹）
  - 微信 JSON 格式可先用 `tools/wechat_parser.py`（如存在）预处理
- **B（台词/文本）**：`Read` 或用户粘贴 → 同样走 `chat_style_analyzer.md` 提取语言风格
- **C（图片）**：`Read` 图片 → 提取 **可视特征** 与 **气质关键词**（冷/暖、锐/钝）
- **D（口述）**：用户口述 → 记关键句与 **禁区**（用户明确不要的内容）
- **E**：标记 `knowledge_sources: []`
...
#### Track C：聊天风格个人化（如果用户导入了聊天记录/台词）

参考 `${CLAUDE_SKILL_DIR}/prompts/chat_style_analyzer.md`：

1. 从导入素材中提取 **用词指纹、句式指纹、情感表达方式、独特表达**
2. 生成 **聊天风格档案**
3. 将档案注入到：
   - `dialogue.md`：所有话术用他的真实说话方式重写
   - `persona.md` Layer 4：语言指纹完全基于档案生成
4. **原则：角色原型决定「做什么」，聊天风格决定「怎么说」，甜度决定「说多甜」**
